...
#	endif
	...
	void test_hazards()
	{
		...
		TEST(__builtin_cheri_tag_get(ptr), "Failed to allocate {} bytes", allocSize);
		void *ptr2 = heap_allocate(&longTimeout, SECOND_HEAP, allocSize);
		TEST(__builtin_cheri_tag_get(ptr2), "Failed to allocate {} bytes", allocSize);
		...
		// freed, the ephemeral claims are released and the allocator
		// is called, causing it to truly free `ptr`. At this stage
		// calling `heap_free` on `ptr` will now return `-EINVAL`
		// (instead of `-EPERM`).
		state = 2;
		sleeps = 0;
		...
		TEST_EQUAL(heap_free(MALLOC_CAPABILITY, ptr2), 0, "Releasing the claim failed");
